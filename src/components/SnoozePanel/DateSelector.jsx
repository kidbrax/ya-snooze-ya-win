    ...
    <SnoozeModal visible={visible}>
      <Root>
        ...
        <MyDayPicker
          ...
          disabled={(date) => moment(date).diff(moment().startOf('day')) < 0}
          showOutsideDays
          fixedWeeks
        />
...
const Navbar = ({ hour, onHourChange, gotoToday, month, onNextClick, onPreviousClick }) => (
  <NavbarDiv>
    <NavButton onClick={() => onPreviousClick()}>
      <img src={leftIcon} alt="Previous Month" />
    </NavButton>
    {/* Function also as Today button */}
    <MonthName onClick={gotoToday}>{moment(month).format('MMMM YYYY')}</MonthName>
    <NavButton onClick={() => onNextClick()}>
      <img src={rightIcon} alt="Next Month" />
    </NavButton>
    <HourOptions value={hour} onChange={onHourChange} style={{ marginLeft: 6 }} />
  </NavbarDiv>
)
