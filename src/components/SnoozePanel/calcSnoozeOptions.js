...
export default function calcSnoozeOptions(settings) {
  const { workdayEnd, weekStartDay, weekEndDay, workdayStart, somedayMonthsDelta, customSnoozeOptions } =
    settings

  const isVeryLateAtNight = moment().hour() <= 3
  const isNightTime = moment().hour() >= workdayEnd || moment().hour() < 3
  const isWeekend = moment().day() === weekEndDay || moment().day() === (weekEndDay + 1) % 7
  ...
  const resolveWhen = (opt) => {
    switch (opt.type) {
      case 'offset':
        return moment()
          .add(opt.offsetMinutes ?? 60, 'minutes')
          .toDate()
      case 'evening':
        return roundDate(
          moment().hour() >= workdayEnd ? moment().add(1, 'day').hour(workdayEnd) : moment().hour(workdayEnd)
        ).toDate()
      case 'tomorrow':
        return (isVeryLateAtNight ? dayStart(moment()) : dayStart(moment().add(1, 'days'))).toDate()
      case 'weekend':
        return (
          isWeekend ? dayStart(moment().day(7 + weekEndDay)) : dayStart(moment().day(weekEndDay))
        ).toDate()
      case 'next_week':
