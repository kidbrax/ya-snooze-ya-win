...
import { createTabs, notifyUserAboutNewTabs, areTabsEqual, getFirstTabToWakeup } from './utils'
...
*/
export async function deleteSnoozedTabs({ tabsToDelete, scheduleAlarm = true }) {
  console.log(
    `🗑️ [${SERVICE_WORKER_INSTANCE_ID}] deleteSnoozedTabs() - Deleting ${tabsToDelete.length} tabs (scheduleAlarm: ${scheduleAlarm})`
  )
...
*/
export async function openTabs({ tabs, makeActive = false }) {
  const result = await createTabs(tabs, makeActive)
  console.log(
    `✅ [${SERVICE_WORKER_INSTANCE_ID}] openTabs() - Created ${result.created.length} browser tabs, ${result.customHandled.length} externally handled, ${result.failedTabs.length} failed`
  )
...
*/
export async function wakeupDeleteAndReschedule({ tabs, makeActive = false }) {
  ...
  console.log(
    `🗑️ [${SERVICE_WORKER_INSTANCE_ID}] wakeupDeleteAndReschedule() - Deleting ${successfulTabs.length} tabs from storage (${customHandled.length} externally handled, ${failedTabs.length} failed, kept)...`
  )
  ...
  console.log(
    `🔁 [${SERVICE_WORKER_INSTANCE_ID}] wakeupDeleteAndReschedule() - Found ${periodicTabs.length} periodic tabs to reschedule`
  )
  for (let tab of periodicTabs) {
    console.log(
      `🔁 [${SERVICE_WORKER_INSTANCE_ID}] wakeupDeleteAndReschedule() - Rescheduling periodic tab: ${tab.url}`
    )
    try {
...
export async function handleScheduledWakeup() {
  ...
  try {
    ...
    let readySleepingTabs = snoozedTabs.filter(
      (snoozedTab) => new Date(snoozedTab.when) <= now && !recentlyWokenKeys.includes(getTabKey(snoozedTab))
    )
    ...
    if (readySleepingTabs.length > 0) {
      ...
      try {
        ...
        console.log(
          `✅ [${SERVICE_WORKER_INSTANCE_ID}] wakeupDeleteAndReschedule() completed - created ${createdTabs.length} browser tabs`
        )
...
export async function scheduleWakeupAlarm(when) {
  ...
  console.log(
    `📊 [${SERVICE_WORKER_INSTANCE_ID}] Found ${snoozedTabs.length} snoozed tabs for alarm scheduling`
  )
  let alarmTime = 0
  ...
  if (when === 'auto') {
    ...
    console.log(
      `🔔 [${SERVICE_WORKER_INSTANCE_ID}] Scheduling alarm for next tab: ${new Date(alarmTime).toISOString()} (${nextTabToWakeup.url})`
    )
  } else {
    // when === '1min'
    alarmTime = Date.now() + 1000 * 60
    console.log(
      `🔔 [${SERVICE_WORKER_INSTANCE_ID}] Scheduling alarm for 1 minute from now: ${new Date(alarmTime).toISOString()}`
    )
  }
...
export function registerEventListeners() {
  ...
  chrome.alarms.onAlarm.addListener(async function (alarm) {
    console.log(
      `🔔 [${SERVICE_WORKER_INSTANCE_ID}] ALARM FIRED: "${alarm.name}" at ${new Date(alarm.scheduledTime).toISOString()}`
    )
